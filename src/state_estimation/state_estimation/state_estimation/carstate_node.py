...
class Carstate(Node):
    def __init__(self):
        ...
        # ros params
        # Velocity source. The pose comes from TF (localisation); only the twist
        # is taken from here. /vesc/odom is the wheel odometry - fine above the
        # VESC's openloop floor, which is where racing happens. Point this at a
        # fused topic instead if the velocity turns out too noisy to control on.
        self.declare_parameter('/carstate_node/odom_topic', "/vesc/odom")
        ...
        # Upstream hardcodes "map" and "base_link"; ours are ego_racecar/*.
        self.declare_parameter('/carstate_node/map_frame', "map")
        self.declare_parameter('/carstate_node/base_frame', "ego_racecar/base_link")
        ...
        self.map_frame = self.get_parameter('/carstate_node/map_frame').value
        self.base_frame = self.get_parameter('/carstate_node/base_frame').value
    ...
    def get_slam_tf(self) -> TransformStamped:
        trans = self.tf_buffer.lookup_transform(
            ...
            rclpy.duration.Duration(seconds=6.9))
        return trans
    ...
    def cartesian_state_loop(self):
        ...
        try:
            # Short timeout on purpose: this runs at 80 Hz, and upstream's 6.9 s
            # would stall the executor for hundreds of cycles whenever
            # localisation drops out.
            trans = self.tf_buffer.lookup_transform(
                self.map_frame, self.base_frame, rclpy.time.Time(),
                rclpy.duration.Duration(seconds=0.1))
        except Exception as e:
            self.get_logger().warn(f"{self.map_frame} -> {self.base_frame} unavailable: {e}",
